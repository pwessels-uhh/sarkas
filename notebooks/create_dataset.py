...
Nps = np.array([10000], dtype = np.int)
data = pd.DataFrame()

example_folder = os.path.join('..',
                              os.path.join('sarkas', os.path.join('examples', 'PreProcParams') ))
# COULOMB
t0 = stimer.current()
for inps, nps in enumerate(Nps):
    flename = os.path.join(example_folder, "ocp_pppm_N{}k.yaml".format(int(nps*1e-3)) )
    ...
    print('Doing {}'.format(nps))
    for im, mesh in enumerate(meshes):
        alphas = np.linspace(0.1, 1.0, 10) /preproc.parameters.a_ws
...
print("Coulomb done")    
data.to_csv('coulomb_dataset.csv', index=False)
tend = stimer.current()
print(tend-t0)
...
#
# for inps, nps in enumerate(Nps):
#     flename = os.path.join('input', "yocp_pppm_N{}k.yaml".format(int(nps*1e-3)) )
#     preproc = PreProcess(flename)
#     preproc.setup(read_yaml=True)
#     max_cells = int(0.5 * preproc.parameters.box_lengths.min() / preproc.parameters.a_ws)
#     pp_cells = np.arange(3, max_cells, dtype= np.int)
...
#         for im, mesh in enumerate(meshes):
#             alphas = np.linspace(0.1, 2.0, 39) /preproc.parameters.a_ws
#             for ia, alpha in enumerate(alphas):
#                 for ir, rc in enumerate(rcs):
#                     for cao in range(7):
#                         new_args = {"Potential":
...
#                                 'coupling_constant': preproc.parameters.coupling_constant,
#                                 'a_ws': preproc.parameters.a_ws,
#                                 'kappa': preproc.parameters.a_ws/preproc.parameters.lambda_TF,
#                                 'rc': preproc.potential.rc,
#                                 'pp_cells': int(preproc.parameters.box_lengths.min()/preproc.potential.rc),
#                                 'Mx': preproc.potential.pppm_mesh[0],
#                                 'My': preproc.potential.pppm_mesh[1],
